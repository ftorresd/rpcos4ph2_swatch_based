...
And<T>::And( const Left& aLeft, const Right& aRight ) {
	BOOST_STATIC_ASSERT_MSG( (std::is_base_of<XRule<T>, Left>::value) , "class Left in And( const Left& aLeft, const Right& aRight ) must be a descendant of XRule<T>" );
	BOOST_STATIC_ASSERT_MSG( (std::is_base_of<XRule<T>, Right>::value) , "class Left in And( const Left& aLeft, const Right& aRight ) must be a descendant of XRule<T>" );
...
// ----------------------------------------------------------------------------
template<typename T>
XMatch And<T>::verify( const T& aValue ) const 
{
