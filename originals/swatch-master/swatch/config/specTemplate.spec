...
%define _tmppath /tmp
%define _htdocspath  __htdocspath__
...
if [ "${#DEBUGDIR}" -gt "${#PKGDIR}" ]; then
  ...
  (>&2 echo "Please make base directory path of source code longer in order to build RPMs.")
  exit 1
fi
...
# find . -name "*" -exec install -D -m 755 {} $RPM_BUILD_ROOT/%{_prefix}/bin/{} \;
  find . -name "*" -exec $BUILD_HOME/config/install.sh {} %{_prefix}/bin/%{_project}/{} 755 $RPM_BUILD_ROOT %{_packagedir} %{_packagename} %{_version} %{_prefix}/include '%{_includedirs}' \;
fi
...
if [ -d %{_packagedir}/include ]; then
  ...
  find . \( -name "*.h" -o -name "*.hh" -o -name "*.hpp"  -o -name "*.hxx" \)  -exec install -D -m 644 {} $RPM_BUILD_ROOT/%{_prefix}/include/{} \;
fi
...
if [ -d %{_packagedir}/html ]; then
  cd %{_packagedir}/html; \
  find . -type f -exec install -D -m 644 {} $RPM_BUILD_ROOT/%{_htdocspath}/html/{} \;
fi
